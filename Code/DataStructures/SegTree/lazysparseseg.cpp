...
struct LazySparseSeg{ 
	...
	UT(S, T); UT(S, L); //value and lazy types
	...
	void make_children(P i){if (!v[i].lc)v[i].lc = new_node(), v[i].rc = new_node();}
	void put(L x, P i){v[i].lz = v[i].ig ? x : S::cmp(v[i].lz, x); v[i].ig = 0;}
